...
public class HomeNewViewController : ViewControllerBase
{
    ...
    public void UpdateLatestPieChart()
    {
        ...
        if (latestSleepDatas != null)
        {
            ...
            if(chartInfo != null)
            {
                float[] pieValues = new float[4] { chartInfo.pFumei, chartInfo.pMukokyu, chartInfo.pIbiki, chartInfo.pKaiMin }; //Percents of Fumei, Mukokyu, Ibiki, pKaiMin
                string[] pieLabels = new string[4] { "不明", "無呼吸", "いびき", "快眠" };
                makePieChart(pieValues, pieLabels);
            }
        }
    }

    public void makePieChart(float[] pieValues, string[] pieLabels)
    {
        float total = 0f;
        float numPie = pieValues.Length;
